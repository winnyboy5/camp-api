...
def create():
    try:
        ...
        if request.json.get("base_card", None):
            cardCheck = Post.query.filter_by(user_id=claims['id'], base_card=True).first()
            print(cardCheck)
            if cardCheck:
                return jsonify(
                status='DENIED',
...
def fetch_cards():
    ...
    return jsonify(
        status='saved',
        data=[Post.serialize() for card in cards],
    ), 200
...
def upload_file():
    claims = get_jwt()
    print(request)
    cardUpdate = Post.query.filter_by(id=request.form.get('card_id')).first()
